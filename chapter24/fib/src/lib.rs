use napi_ohos::{CallContext, Env, JsNumber, JsObject, Result, Task};
...
pub fn native_fib(ctx: CallContext) -> Result<JsObject> {
  let task = ComputeFib::new(45);
  let async_promise = ctx.env.spawn(task).unwrap();
  Ok(async_promise.promise_object())
}

#[module_exports]
pub fn register_js(mut exports: JsObject) -> Result<()> {
  exports.create_named_method("fib", native_fib).unwrap();
  Ok(())
}
